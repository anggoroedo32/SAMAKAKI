...
class BuchheimWalkerLayoutManager @JvmOverloads constructor(private val context: Context, val configuration: BuchheimWalkerConfiguration = BuchheimWalkerConfiguration.Builder().build()) : GraphLayoutManager(context) {
    private val mNodeData: MutableMap<com.awp.samakaki.utils.Node, BuchheimWalkerNodeData> = HashMap()
    ...
    private fun getNodeData(node: com.awp.samakaki.utils.Node): BuchheimWalkerNodeData {
        Log.d("LogNodeData", "data:${node.data},x is ${node.x},y is${node.y}, position is ${node.position}")
        return mNodeData.getValue(node)
    }

    private fun firstWalk(graph: com.awp.samakaki.utils.Graph, node: com.awp.samakaki.utils.Node, depth: Int, number: Int) {
    ...
    private fun secondWalk(graph: com.awp.samakaki.utils.Graph, node: com.awp.samakaki.utils.Node, modifier: Double) {
    ...
    private fun isLeaf(graph: com.awp.samakaki.utils.Graph, node: com.awp.samakaki.utils.Node): Boolean {
    ...
    override fun run(graph: com.awp.samakaki.utils.Graph, shiftX: Float, shiftY: Float): Size {
