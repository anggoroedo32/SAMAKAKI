...
class BuchheimWalkerLayoutManager @JvmOverloads constructor(private val context: Context, val configuration: BuchheimWalkerConfiguration = BuchheimWalkerConfiguration.Builder().build()) : GraphLayoutManager(context) {
    private val mNodeData: MutableMap<com.qatros.samakaki.utils.Node, BuchheimWalkerNodeData> = HashMap()
    ...
    private fun getNodeData(node: com.qatros.samakaki.utils.Node): BuchheimWalkerNodeData {
        Log.d("LogNodeData", "data:${node.data},x is ${node.x},y is${node.y}, position is ${node.position}")
        return mNodeData.getValue(node)
    }

    private fun firstWalk(graph: com.qatros.samakaki.utils.Graph, node: com.qatros.samakaki.utils.Node, depth: Int, number: Int) {
    ...
    private fun secondWalk(graph: com.qatros.samakaki.utils.Graph, node: com.qatros.samakaki.utils.Node, modifier: Double) {
    ...
    private fun isLeaf(graph: com.qatros.samakaki.utils.Graph, node: com.qatros.samakaki.utils.Node): Boolean {
    ...
    override fun run(graph: com.qatros.samakaki.utils.Graph, shiftX: Float, shiftY: Float): Size {
